  ...
  describe("Auction with reserve price", () => {
    ...
    it("cannot create an auction for a token I do not own", async () => {
      ...
      try {
        ...
        expect(
          message.includes("can't start the auction of NFT you don't own")
        );
      }
    });
    it("cannot create an auction for a token that already has an ongoing auction", async () => {
      ...
      try {
        ...
        const message = new String(e);
        expect(message.includes("previous auction is still on going."));
      }
    });
    ...
    it("cannot bid on an auction if my price is lower than the reserve price", async () => {
      ...
      try {
        ...
        const message = new String(e);
        expect(
          message.includes("bid amount should be greator than the reservePrice")
        );
      }
    });
    ...
    it("cannot end an ongoing auction before the end time", async () => {
      ...
      try {
        ...
        const message = new String(e);
        expect(message.includes("can`t end ongoing time based auction"));
      }
    });
    ...
    it("cannot end an auction I did not create", async () => {
      ...
      try {
        ...
        const message = new String(e);
        expect(message.includes("only owner of the NFT can end the auction"));
      }
    });
    ...
    it("cannot withdraw my funds out if the auction is ongoing", async () => {
      ...
      try {
        ...
        const message = new String(e);
        expect(
          message.includes("can't withdraw money until the auction has ended")
        );
      }
    });
    it("cannot withdraw my funds out if I did not put money in", async () => {
      ...
      try {
        ...
        const message = new String(e);
        expect(
          message.includes("you don't have any money in the bidding pool")
        );
      }
    });
